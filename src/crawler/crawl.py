...
# local
from src.crawl_utils import whether_to_crawl, get_classes
...
def save_pretty_html(source: str, filename: str):
    soup = BeautifulSoup(source, "html.parser")
    pretty_html = soup.prettify()
    with open(filename, "w", encoding="utf-8") as f:
        f.write(pretty_html)
    logger.info(f"✅ Pretty HTML saved: {filename}")
...
if __name__ == "__main__":
    ...
    save_pretty_html(source, "./source.html")
    tags = get_content(source=source, instruction="data-name이 REVIEW인 클래스를 찾아줘")
    print(tags)